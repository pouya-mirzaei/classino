...
export default function Timer() {
  const { days, hours, minutes, seconds, delta } = useCountDownTimer('7 july 2025 03:30:00');

  const percentageLeft = Math.ceil(((delta / (365 * 24 * 3600 * 1000)) * 100).toString().split('').slice(0, 4).join(''));
    ...
    <div className="basis-7/12 w-full bg-white dark:bg-dark-1 rounded-xl shadow-md shadow-black/20">
      <div className="p-5">
        ...
        <div className="flex flex-col justify-center items-center">
          <div className="my-6">
            <h3 className="text-sm text-center dark:text-white font-semibold">{percentageLeft}% از زمان شما باقی مانده </h3>
          </div>
          <div className="relative w-[90%] h-5 bg-gray-300 dark:bg-dark-3 rounded-xl">
            <div
              ...
              style={{ width: Number(percentageLeft) + '%' }}
            ></div>
          </div>
        </div>
      </div>
